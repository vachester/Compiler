...
void Parser::get_closure(Status &p) {
	bool change = true;
	Status pptmp;
	std::unordered_set<int> record; //记录已经求过的产生式
	while(change) {
		change = false;
		pptmp = p;
		for(auto &pro : pptmp) {
			if(!(record.find(pro.pro_num) == record.end() && pro.dot_position < static_cast<int>(G[pro.pro_num].right.size()))) 
				continue;
			record.insert(pro.pro_num);
			...
			for(auto &i : index[V]) {
				...
				if(p.find(ptmp) == p.end()) {
					...
					auto original = new_successor.size(); 
...
/*
   for(auto jj : p) {
   std::cout << jj.pro_num << " " << jj.dot_position << ",";
   for(auto kk : jj.successors)
   std::cout << kk << " ";
   std::cout << std::endl;
   }
   */
...
bool Parser::judge_repeat(Status s1, Status s2) {
	if(s1.size() == s2.size()) {
		for(auto pros : s1) {
